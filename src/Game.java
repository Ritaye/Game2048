...
public class Game extends JPanel{
	...
	private void doMove(int x,int y,String direction) {
	...
	private void doMerge(int x,int y,int tX,int tY,String direction) {
	...
	private boolean canMerge(int x,int y,String direction) {
		switch (direction) {
		case "left":
	...
	private Point getPoint(Tile[][] f,Tile t) {
		for (int i=0;i<4;i++)
			for (int j=0;j<4;j++)
				if (f[i][j] == t)
	...
	private void setDefaultTile(Tile[][] t){
		for (int i=0;i<4;i++)
			for (int j=0;j<4;j++)
				t[i][j].setDefault();
	}
	private Tile[][] newTile(Tile[][] old){
		Tile[][] t = new Tile[4][4];
		for (int i=0;i<4;i++)
			t[i] = new Tile[4];
		for (int i=0;i<4;i++)
	...
	private void drawTile(Graphics g2,Tile tile,int x,int y,int moveX,int moveY,int enlarge) {
